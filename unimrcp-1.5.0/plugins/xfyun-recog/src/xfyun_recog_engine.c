...
{
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 5e61adb4, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
...
static apt_bool_t xfyun_recog_stream_recog(xfyun_recog_channel_t *recog_channel,
	...
	while (1) 
	{
		...
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"[xfyun] Get recog result:%s:%d",rslt, rec_stat);
...
static apt_bool_t xfyun_recog_stream_write(mpf_audio_stream_t *stream, const mpf_frame_t *frame)
{
	...
	if(frame->codec_frame.size) {
		ret = xfyun_recog_stream_recog(recog_channel, frame->codec_frame.buffer, frame->codec_frame.size);
		if(ret == MSP_ERROR_TIME_OUT){
	...
	if(recog_channel->recog_request) {
		...
		switch(det_event) {
			...
			case MPF_DETECTOR_EVENT_NOINPUT:
				...
				break;
			default:
				//apt_log(RECOG_LOG_MARK,APT_PRIO_NOTICE,"Detector Event " APT_SIDRES_FMT,MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
				break;
		}
